...
static void	put_nseq(Sequence *, int, int, int);
...
doless:
	...
	{	fprintf(tt, "#ifndef PEG\n");
		...
		fprintf(tt, "\textern int *SI;\n");
		fprintf(tt, "\textern int *sicnt;\n");
		fprintf(tt, "\textern int xspinflag, numberofbastates;\n");
...
/* end */
	...
	if (p->b == N_CLAIM){
	  fprintf(tt, "\n  SI = (int *)");
	  fprintf(tt, " emalloc(%d*sizeof(int));\n\n", p->s->maxel);
	  put_nseq(p->s, 2, 0, XSI);
	  fprintf(tt, "\n sicnt = (int *)");
	  fprintf(tt, " emalloc(%d*sizeof(int));\n\n", p->s->maxel);
	  fprintf(tt, "for(i=0;i<%d;i++)", p->s->maxel);
	  fprintf(tt, "\t sicnt[i]=0;");
	  fprintf(tt, "xspinflag = 1;");
	  fprintf(tt, "numberofbastates = %d;", p->s->maxel);
	}
	else 
	  put_seq(p->s, 2, 0);
        #else
...
#ifdef XSPIN
static void
put_nseq(Sequence *s, int Tt0, int Tt1, int si_flag)
{	SeqList *h;
	Element *e, *g;
	int a, deadlink;
	...
	for (e = s->frst; e; e = e->nxt)
	{
		...
		} else if (e->sub)
		{
		  ...
		  fprintf(tt, "\tSI[%d] = 0;\n", e->seqno);
			...
			for (h = e->sub; h; h = h->nxt)
			  put_nseq(h->this, Tt0, Tt1,siflag);
		} else
		{
