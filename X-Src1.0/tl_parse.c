...
detect_SI_subformulae(Node *n){
  ...
  case PREDICATE:
  case NOT:
    detect_SI_subformulae(n->lft);
    detect_SI_subformulae(n->rgt);
    if (n->lft!=0 && n->lft->si_flag==XS)
      n->si_flag = XS;
    else if (n->rgt!=0 && n->rgt->si_flag==XS)
      n->si_flag = XS;
    else n->si_flag = SI;
    break;
  case NEXT:
  ...
  case OR:
    detect_SI_subformulae(n->lft);
    detect_SI_subformulae(n->rgt);
    n->si_flag = ((n->lft->si_flag==SI)&&(n->rgt->si_flag==SI)?SI:XS);
    break;
  default:
    break;
  }
