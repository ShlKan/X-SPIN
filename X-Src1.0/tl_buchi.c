...
/*modified by Shuanglong Kan for X-SPIN*/
...
printstate(State *b)
{
	...
	#ifdef XSPIN
	  // if (analyze != 1)
	  fprintf(tl_out, "T0%s %s :\n", &(b->name->name[6]),(b->si_flag==SI?" ":" @ "));
	//  else
	//  fprintf(tl_out, "T0%s:\n", &(b->name->name[6]));
	#else
	  fprintf(tl_out, "T0%s:\n", &(b->name->name[6]));
...
{	State *b;
	...
	*/
	...
	#ifdef XSPIN
	b->si_flag = op->si_flag;
	/*
	  si_flag = 1;
	for(tmp = col->New; tmp && (si_flag==1); tmp=tmp->nxt){
	  if (tmp->si_flag == XS){
	    si_flag = 0;
	    break ;
	  }
	}
	for(tmp = col->Old; tmp && (si_flag==1); tmp=tmp->nxt){
	  if(tmp->si_flag == XS){
	    si_flag = 0;
	    break;
	  }
